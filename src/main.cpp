...
// ---- Tum kareler icin feature extraction + pencereli VIO (IMU seed + Ceres BA) + canli gorsellestirme ----
void processFrames(const ImageLoader& images, ORBExtractor& extractor, FeatureMatcher& matcher,
                    const cv::Mat& K, const cv::Mat& R_BS,
                    const cv::Mat& undistortMap1, const cv::Mat& undistortMap2,
                    ImuPreintegrator& imuPreint, VIOBundleAdjustment& vio,
                    ...
                    int& windowFailCount, int& windowTotalCount, int& priorSuccessCount,
                    KeyframeDatabase& kfDb, std::vector<LoopVerificationResult>& foundLoops) {

    // ---- Pencere biriktiricileri ----
    ...
    // ---- Zincir capasi: baslangicta kare-0 = referans/orijin ----
    VIOBundleAdjustment::FrameState anchor;
    anchor.pose = {0,0,0,0,0,0};
    anchor.vel  = {0,0,0};
    anchor.ba   = {0,0,0};
    anchor.bg   = {0,0,0};
    // ---- Marjinalizasyon yaklasiklamasi: capanin poz+hiz belirsizligi (bkz. WindowPrior) - ilk
    // pencerede valid=false, BA'nin sonraki her basarili cozumunde guncellenir ----
    ...
    for (size_t i = 0; i < images.imageFiles.size(); ++i) {
        ...
        // ---- Distorsiyon duzeltmesi: feature'lar/triangulasyon/reprojection PINHOLE (distorsiyonsuz)
        // varsayiyor; ham EuRoC goruntusu distorsiyonlu oldugundan burada duzeltiliyor (bkz. buildDistortionCoeffs) ----
        ...
        // ---- IMU dead-reckoning ile o ana kadarki pencereyi capa uzerinden seed'le (canli gorsellestirme icin) ----
        ...
        // buildFullCoverageTracks artik HER karede cagriliyor (minWindowFrames'e ulasilinca) - kamera
        // yeterince hareket edene (paralaksThresholdPx) ya da guvenlik tavanina (maxWindowFrames)
        // ulasana kadar pencere acik kalir. Boylece dusuk-hareketli donemlerde triangulasyonun
        // geometrik olarak dejenere oldugu (TUM izlerin cheirality'den elendigi) durumlar azaltilir.
        if ((int)kpList.size() >= minWindowFrames) {
            ...
            // GUVENME, sadece maxWindowFrames tavanina gelince kapat (eskisi gibi guvenli geri donus).
            ...
            if (enoughParallax || reachedMax) {
                ...
                // Kullanicinin onerisinin YUMUSAK versiyonu: BA'yi TAMAMEN atlamak (denendi, ATE'yi
                // 1.94m -> 2.19m'den KATMERLI kotulestirdi, |v| 61 m/s'ye |C| 582m'ye kacti - zayif
                // pencerelerin CoGUNLUGU aslinda iyi calisiyormus, hepsini atmak zarar verdi) yerine:
                // BA'yi HER ZAMAN calistir, ama dusuk-guven (MAKS-KARE limiti) pencerelerde optimizorun
                // hizi kacirma serbestligini SIKI bir hiz prior'uyla daralt - "kamera guvenilir degilse
                // IMU tahminine daha siki sarilalim" ilkesinin gorsel duzeltmeyi tamamen atmayan hali.
                ...
                std::cout << "Pencere " << windowCount << " (" << framesUsed << " kare, paralaks="
                          << parallaxPx << "px" << (lowConfidenceForceClose ? ", MAKS-KARE limiti, siki-hiz-prior" : "")
                          << "): " << tracks.size() << " tam kapsama izi, ";

                if (!tracks.empty()) {
                    std::vector<VIOBundleAdjustment::FrameState> optStates = windowStates;
                    int64 tBaStart = cv::getTickCount();
                    ok = vio.optimize(optStates, frameTimestamps, kpList, tracks, landmarks,
                                       currentPrior.valid ? &currentPrior : nullptr, &outPrior, velPriorWeight);
                    double baMs = (cv::getTickCount() - tBaStart) / cv::getTickFrequency() * 1000.0;
                    std::cout << "(BA+kov: " << baMs << " ms) ";

                    if (ok) {
                        // isfinite tek basina yetmiyor: teshis edilen vaka (bkz. Adim 7) BA'nin NaN/Inf
                        // URETMEDEN, ama fiziksel olarak SACMA (|v|=26 m/s, sonra 115 m/s) bir cozume
                        // "basarili" olarak yakinsayabildigini gosterdi. Gevsek ama makul bir ust sinir:
                        // bu dataset'te (EuRoC MH01) gozlenen gercek hizlar hep <5 m/s, 15 m/s bol pay birakir.
                    ...
                    if (ok) {
                        anchor = optStates.back();
                        currentPrior = outPrior; // outPrior.valid=false ise sonraki pencere sert-sabite doner
                        if (outPrior.valid) priorSuccessCount++;
                        std::cout << "BA basarili=true\n";

                        // ---- Loop closure: keyframe ekle + yer tanima + PnP dogrulama ----
                        // (bkz. pip06_LoopClosure.hpp) - landmarks/tracks BURADA zaten hesaplanmis,
                        // YENIDEN HESAPLAMA yok. Pose graph SADECE calisma sonunda (main()'de) cozulur.
                        Keyframe kf;
                        kf.frameIndex = (int)i;
                        kf.pose = optStates.back().pose;
                        kf.keypoints = kpList.back();
                        kf.descriptors = descList.back();
                        int lastFrameIdx = (int)kpList.size() - 1;
                        for (size_t ti = 0; ti < tracks.size(); ++ti) {
                            kf.landmarks.emplace_back(landmarks[ti][0], landmarks[ti][1], landmarks[ti][2]);
                            kf.landmarkKptIdx.push_back(tracks[ti][lastFrameIdx]);
                        }
                        if (kfDb.addKeyframe(kf)) {
                            int newKfDbIdx = (int)kfDb.keyframes.size() - 1;
                            std::vector<int> candidates = findLoopCandidates(kfDb, matcher);
                            for (int cIdx : candidates) {
                                LoopVerificationResult lv = verifyLoopPnP(kfDb, cIdx, kfDb.keyframes.back(), newKfDbIdx, matcher, K);
                                if (lv.valid) {
                                    std::cout << "  [LOOP] Dogrulanan dongu: keyframe " << cIdx << " (kare "
                                              << kfDb.keyframes[cIdx].frameIndex << ") <-> yeni keyframe (kare "
                                              << i << "), inlier=" << lv.inlierCount << "\n";
                                    foundLoops.push_back(lv);
                                }
                            }
                        }
                    } else {
                        anchor = windowStates.back();
                        currentPrior.valid = false; // mevcut IMU-seed fallback'iyle BIREBIR ayni mantik
                        std::cout << "BA basarili=false (UYARI: BA sonucu kullanilamadi/NaN/mantiksiz hiz, IMU seed korunuyor)\n";
                        windowFailCount++;
                    }
                } else {
                    anchor = windowStates.back();
                    currentPrior.valid = false;
                    std::cout << "BA basarili=false (UYARI: pencerede hicbir iz tum kareleri kapsamadi)\n";
                    windowFailCount++;
                }

                // ---- TESHIS: capa hiz/konum buyuklugu - kacis nerede basliyor gormek icin ----
                {
                    double velNorm = std::sqrt(anchor.vel[0]*anchor.vel[0] + anchor.vel[1]*anchor.vel[1] + anchor.vel[2]*anchor.vel[2]);
                    cv::Point3d c = cameraCenterFromPose(anchor.pose);
                    double posNorm = std::sqrt(c.x*c.x + c.y*c.y + c.z*c.z);
                    double baNorm = std::sqrt(anchor.ba[0]*anchor.ba[0] + anchor.ba[1]*anchor.ba[1] + anchor.ba[2]*anchor.ba[2]);
                    std::cout << "  [TESHIS] capa: |v|=" << velNorm << " m/s, |C|=" << posNorm
                              << " m, |ba|=" << baNorm << " m/s^2, kare=" << framesUsed
                              << ", paralaks=" << parallaxPx << "px, capa_prior="
                              << (currentPrior.valid ? "var" : "yok") << "\n";
                }
...
int main() {
    ...
    int windowFailCount = 0, windowTotalCount = 0, priorSuccessCount = 0;
    processFrames(images, extractor, matcher, K, R_BS, undistortMap1, undistortMap2,
                  ...
                  windowFailCount, windowTotalCount, priorSuccessCount, kfDb, foundLoops);

    std::cout << "Pencere ozeti: " << (windowTotalCount - windowFailCount) << "/" << windowTotalCount
              << " basarili (" << windowFailCount << " basarisiz), " << priorSuccessCount
